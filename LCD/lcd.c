...
#if defined(__AVR_ATmega64__) || defined(__AVR_ATmega64A__) || defined(__AVR_ATmega128__)
...
void lcd_putc_hex( unsigned char c)                          // 1 hexadecimalis szamjegy kiirasa
{
	c&= 0x0F;
	if( c < 10) c+= '0';
	else        c+= 'A'- 10;
...
void lcd_putbyte_hex( unsigned char c)                       // 1 byte kiirasa hexadecimalisan
{
	lcd_putc_hex( c >> 4);
	lcd_putc_hex( c);
}
